...
This directory has the code for the Opentrons STM32 reference module.
...
- Build the firmware: `cmake --build ./build-stm32-cross --target reference-module`
- Build the firmware, connect to a debugger, and upload it: `cmake --build ./build-stm32-cross --target reference-module-debug`
- Lint the firmware: `cmake --build ./build-stm32-cross --target reference-module-lint`
- Format the firmware: `cmake --build ./build-stm32-cross --target reference-module-format`
- Flash the firmware to a board: `cmake --build ./build-stm32-cross --target reference-module-flash`
- Flash the firmware __and__ the startup application to a board: `cmake --build ./build-stm32-cross --target reference-module-image-flash`
- Builds reference-module-image.hex, suitable for use with stm's programmer: `cmake --build ./build-stm32-cross --target reference-module-image-hex`
- Builds reference-module-image.bin, suitable for some other programmers: `cmake --build ./build-stm32-cross --target reference-module-image-bin`
- Build the startup app, which is also packaged into the image files: `cmake --build ./build-stm32-cross --target reference-module-startup`
- Delete all of the contents on a reference-module MCU, and wipe any memory protection: `cmake --build ./build-stm32-cross --target reference-module-clear`
...
- Build tests: `cmake --build ./build-stm32-host --target reference-module-tests`
- Run tests: `cmake --build ./build-stm32-host --target test`
- Format tests: `cmake --build ./build-stm32-host --target reference-module-format`
- Build simulator: `cmake --build ./build-stm32-host --target reference-module-simulator` 
- Build and Test: `cmake --build ./build-stm32-host --target reference-module-build-and-test` 
