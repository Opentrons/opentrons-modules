# Vacuum Module Firmware

This directory has the code for the Opentrons STM32 vacuum module.
...
- Build the firmware: `cmake --build ./build-stm32-cross --target vacuum-module`
- Build the firmware, connect to a debugger, and upload it: `cmake --build ./build-stm32-cross --target vacuum-module-debug`
- Lint the firmware: `cmake --build ./build-stm32-cross --target vacuum-module-lint`
- Format the firmware: `cmake --build ./build-stm32-cross --target vacuum-module-format`
- Flash the firmware to a board: `cmake --build ./build-stm32-cross --target vacuum-module-flash`
- Flash the firmware __and__ the startup application to a board: `cmake --build ./build-stm32-cross --target vacuum-module-image-flash`
- Builds vacuum-module-image.hex, suitable for use with stm's programmer: `cmake --build ./build-stm32-cross --target vacuum-module-image-hex`
- Builds vacuum-module-image.bin, suitable for some other programmers: `cmake --build ./build-stm32-cross --target vacuum-module-image-bin`
- Build the startup app, which is also packaged into the image files: `cmake --build ./build-stm32-cross --target vacuum-module-startup`
- Delete all of the contents on a vacuum-module MCU, and wipe any memory protection: `cmake --build ./build-stm32-cross --target vacuum-module-clear`
...
- Build tests: `cmake --build ./build-stm32-host --target vacuum-module-tests`
- Run tests: `cmake --build ./build-stm32-host --target test`
- Format tests: `cmake --build ./build-stm32-host --target vacuum-module-format`
- Build simulator: `cmake --build ./build-stm32-host --target vacuum-module-simulator` 
- Build and Test: `cmake --build ./build-stm32-host --target vacuum-module-build-and-test` 
