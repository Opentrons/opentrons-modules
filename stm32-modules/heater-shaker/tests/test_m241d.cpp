...
SCENARIO("GetPlateLockStateDebug (M241.D) response works", "[gcode][parse][M241.D]") {
    GIVEN("a response buffer large enough for the formatted response") {
        std::string buffer(64, 'c');
        WHEN("filling response") {
            auto written = gcode::GetPlateLockStateDebug::write_response_into(
                buffer.begin(), buffer.end(), std::array<char, 14> {"hello"}, true, false);
            THEN("the response should be written in full") {
                std::string ok = "M241.D STATE:hello OpenSensor:1 ClosedSensor:0 OK\n";
    ...
    GIVEN("a response buffer not large enough for the formatted response") {
        ...
        WHEN("filling response") {
            auto written = gcode::GetPlateLockStateDebug::write_response_into(
                buffer.begin(), buffer.begin() + 16, std::array<char, 14> {"hello"}, true, false);
