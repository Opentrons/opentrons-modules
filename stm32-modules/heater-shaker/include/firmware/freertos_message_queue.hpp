/*
 * implementation of the MessageQueue concept that uses freertos queues underneath
 */
...
class FreeRTOSMessageQueue {
    public:
        ...
        {}
        // Since the FreeRTOS queue control structures intern data and we don't want to
        // mess with their internals, you cannot copy or move this. It should be declared
        // once, and passed around by reference thereafter.
        FreeRTOSMessageQueue(const FreeRTOSMessageQueue& other) = delete;
        auto operator=(const FreeRTOSMessageQueue& other) -> FreeRTOSMessageQueue& = delete;
        FreeRTOSMessageQueue(const FreeRTOSMessageQueue&& other) noexcept = delete;
        auto operator=(FreeRTOSMessageQueue&& other)  noexcept -> FreeRTOSMessageQueue = delete;
        ~FreeRTOSMessageQueue() = default;
        [[nodiscard]] auto try_send(const Message& message, const uint32_t timeout_ticks=0) -> bool {
