...
/* TF_REGULATION_RATE_SCALED is TF_REGULATION_RATE divided by PWM_FREQ_SCALING
 * to allow more dynamic */
#define TF_REGULATION_RATE_SCALED          \
    (uint16_t)((uint32_t)(PWM_FREQUENCY) / \
               (REGULATION_EXECUTION_RATE * PWM_FREQ_SCALING))

/* DPP_CONV_FACTOR is introduce to compute the right DPP with
 * TF_REGULATOR_SCALED  */
...
#define INRUSH_CURRLIMIT_DELAY_COUNTS                            \
    (uint16_t)(INRUSH_CURRLIMIT_DELAY_MS *                       \
                   ((uint16_t)SPEED_LOOP_FREQUENCY_HZ) / 1000u - \
               1u)

#define MF_TASK_OCCURENCE_TICKS \
    (SYS_TICK_FREQUENCY / SPEED_LOOP_FREQUENCY_HZ) - 1u
...
#define MAX_APPLICATION_SPEED_UNIT \
    ((MAX_APPLICATION_SPEED_RPM * SPEED_UNIT) / _RPM)
#define MIN_APPLICATION_SPEED_UNIT \
    ((MIN_APPLICATION_SPEED_RPM * SPEED_UNIT) / _RPM)

#define MAX_APPLICATION_SPEED_UNIT2 \
    ((MAX_APPLICATION_SPEED_RPM2 * SPEED_UNIT) / _RPM)
#define MIN_APPLICATION_SPEED_UNIT2 \
    ((MIN_APPLICATION_SPEED_RPM2 * SPEED_UNIT) / _RPM)
...
#define OV_TEMPERATURE_THRESHOLD_d \
    ((V0_V + DELTA_V_THRESHOLD) * INT_SUPPLY_VOLTAGE)
...
#define PWM_PERIOD_CYCLES                             \
    (uint16_t)(ADV_TIM_CLK_MHz * (uint32_t)1000000u / \
...
#define SAMPLING_TIME                          \
    ((ADC_SAMPLING_CYCLES * ADV_TIM_CLK_MHz) / \
     ADC_CLK_MHz) /* In ADV_TIMER CLK cycles*/
#define HTMIN                                                                 \
    1 /* Required for main.c compilation only, CCR4 is overwritten at runtime \
       */
#define TW_BEFORE                                                      \
    ((uint16_t)((ADC_TRIG_CONV_LATENCY_CYCLES + ADC_SAMPLING_CYCLES) * \
         ...
         ADC_CLK_MHz +                                                 \
     1u)
#define TW_BEFORE_R3_1                                                    \
    ((uint16_t)((ADC_TRIG_CONV_LATENCY_CYCLES + ADC_SAMPLING_CYCLES * 2 + \
                 ADC_SAR_CYCLES) *                                        \
                ADV_TIM_CLK_MHz) /                                        \
         ADC_CLK_MHz +                                                    \
     1u)
#define TW_AFTER \
    ((uint16_t)(((DEADTIME_NS + MAX_TNTR_NS) * ADV_TIM_CLK_MHz) / 1000ul))
...
#define PQD_CONVERSION_FACTOR                      \
    (int32_t)((1000 * 3 * ADC_REFERENCE_VOLTAGE) / \
...
#define MMITABLE                                                              \
    {                                                                         \
        32767, 32390, 32146, 31907, 31673, 31444, 31220, 31001, 30787, 30577, \
            30371, 30169, 29971, 29777, 29587, 29400, 29217, 29037, 28861,    \
            28687, 28517, 28350, 28185, 28024, 27865, 27709, 27555, 27404,    \
            27256, 27110, 26966, 26824, 26685, 26548, 26413, 26280, 26149,    \
            26019, 25892, 25767, 25643, 25521, 25401, 25283, 25166, 25051,    \
            24937, 24825, 24715, 24606, 24498, 24392, 24287, 24183, 24081,    \
            23980, 23880, 23782, 23684, 23588, 23493, 23400, 23307, 23215,    \
            23125                                                             \
    }

#define SAMPLING_CYCLE_CORRECTION \
    0.5 /* Add half cycle required by STM32F303RETx ADC */
