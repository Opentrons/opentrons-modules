...
class MotorTask {
  ...
  private:
    ...
    template <typename Policy>
    auto visit_message(const messages::CheckMotorStartMessage& msg,
                       Policy& policy) -> void {
        int16_t lower_threshold =
            -5;  // make local constant. Just use one +/- variable?
        int16_t upper_threshold = 5;  // make local constant
        auto error = msg.with_error;
        if ((msg.target_rpm != 0) &&
            (policy.get_current_rpm() > lower_threshold) &&
            (policy.get_current_rpm() < upper_threshold)) {
