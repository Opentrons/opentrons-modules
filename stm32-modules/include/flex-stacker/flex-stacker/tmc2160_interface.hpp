...
concept TMC2160InterfacePolicy = requires(Policy& p, MotorID motor_id, MessageT& data) {
    // A function to read & write to a register. addr should include the
    // read/write bit.
    {
        p.tmc2160_transmit_receive(motor_id, data)
...
*/
template <class TMC2160InterfacePolicy>
class TMC2160Interface {
 public:
   TMC2160Interface(TMC2160InterfacePolicy& policy) : _policy(policy) {}
   ...
   auto write(Registers addr, RegisterSerializedType value, MotorID motor_id)
       ...
       return _policy.tmc2160_transmit_receive(motor_id, buffer.value()).has_value();
   }
   ...
   TMC2160InterfacePolicy& _policy;
};
