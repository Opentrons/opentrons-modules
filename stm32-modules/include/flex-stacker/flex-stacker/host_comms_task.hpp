...
class HostCommsTask {
  ...
  private:
    using GCodeParser = gcode::GroupParser<
        ...
        gcode::GetTOFHistogram, gcode::EnableTOFSensor>;
    ...
    using GetTOFHistogramCache = AckCache<8, gcode::GetTOFHistogram>;
  ...
  private:
    ...
    requires std::forward_iterator<InputIt> &&
        std::sized_sentinel_for<InputLimit, InputIt>
    auto visit_gcode(const gcode::GetTOFHistogram& gcode, InputIt tx_into,
                     InputLimit tx_limit) -> std::pair<bool, InputIt> {
        auto id = get_tof_histogram_cache.add(gcode);
        if (id == 0) {
            return std::make_pair(
                false, errors::write_into(tx_into, tx_limit,
                                          errors::ErrorCode::GCODE_CACHE_FULL));
        }
        auto message = messages::GetTOFHistogramMessage{
            .id = id, .sensor_id = gcode.sensor_id};
        if (!task_registry->send(message, TICKS_TO_WAIT_ON_SEND)) {
            auto wrote_to = errors::write_into(
                tx_into, tx_limit, errors::ErrorCode::INTERNAL_QUEUE_FULL);
            get_tof_histogram_cache.remove_if_present(id);
            return std::make_pair(false, wrote_to);
        }
        return std::make_pair(true, tx_into);
    }
    ...
    requires std::forward_iterator<InputIt> &&
        std::sized_sentinel_for<InputLimit, InputIt>
    auto visit_message(const messages::GetTOFHistogramResponse& response,
                       InputIt tx_into, InputLimit tx_limit) -> InputIt {
        auto cache_entry = get_tof_histogram_cache.remove_if_present(
            response.responding_to_id);
        return std::visit(
            [tx_into, tx_limit, response](auto cache_element) {
                using T = std::decay_t<decltype(cache_element)>;
                if constexpr (std::is_same_v<std::monostate, T>) {
                    ...
                    return cache_element.write_response_into(
                        tx_into, tx_limit, response.sensor_id, response.end,
    ...
    GetTOFHistogramCache get_tof_histogram_cache{};
