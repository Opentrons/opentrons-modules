...
class HostCommsTask {
  ...
  private:
    ...
    using GetInstalledCache = AckCache<8, gcode::GetInstalledStatus>;
  ...
  private:
    ...
    requires std::forward_iterator<InputIt> &&
        std::sized_sentinel_for<InputLimit, InputIt>
    auto visit_gcode(const gcode::GetInstalledStatus& gcode, InputIt tx_into,
                     InputLimit tx_limit) -> std::pair<bool, InputIt> {
        auto id = get_installed_cache.add(gcode);
        if (id == 0) {
            return std::make_pair(
                false, errors::write_into(tx_into, tx_limit,
                                          errors::ErrorCode::GCODE_CACHE_FULL));
        }
        auto message = messages::GetInstalledMessage{.id = id};

        if (!task_registry->send(message, TICKS_TO_WAIT_ON_SEND)) {
            auto wrote_to = errors::write_into(
                tx_into, tx_limit, errors::ErrorCode::INTERNAL_QUEUE_FULL);
            get_installed_cache.remove_if_present(id);
            return std::make_pair(false, wrote_to);
        }
        return std::make_pair(true, tx_into);
    }
    ...
    requires std::forward_iterator<InputIt> &&
        std::sized_sentinel_for<InputLimit, InputIt>
    auto visit_message(const messages::GetInstalledResponse& response,
                       InputIt tx_into, InputLimit tx_limit) -> InputIt {
        auto cache_entry =
            get_installed_cache.remove_if_present(response.responding_to_id);
        return std::visit(
            [tx_into, tx_limit, response](auto cache_element) {
                using T = std::decay_t<decltype(cache_element)>;
                if constexpr (std::is_same_v<std::monostate, T>) {
                    ...
                    return cache_element.write_response_into(
                        tx_into, tx_limit, response.installed);
                }
            },
            cache_entry);
    }
    ...
    requires std::forward_iterator<InputIt> &&
        std::sized_sentinel_for<InputLimit, InputIt>
    auto visit_gcode(const gcode::SetMotorStallGuard& gcode, InputIt tx_into,
                     InputLimit tx_limit) -> std::pair<bool, InputIt> {
        auto id = ack_only_cache.add(gcode);
        if (id == 0) {
            return std::make_pair(
        ...
        auto message =
            messages::SetMotorStallGuardMessage{.id = id,
                                                .motor_id = gcode.motor_id,
                                                .enable = gcode.enable,
                                                .sgt = gcode.sgt};
        if (!task_registry->send(message, TICKS_TO_WAIT_ON_SEND)) {
        ...
        return std::visit(
            [tx_into, tx_limit, response](auto cache_element) {
                ...
                if constexpr (std::is_same_v<std::monostate, T>) {
                    ...
                    return cache_element.write_response_into(
                        tx_into, tx_limit, response.sensor_id, response.kind,
                        response.cancelled, response.len);
                }
            },
            cache_entry);
    }
    ...
    auto visit_gcode(const gcode::GetTOFMeasurement& gcode, InputIt tx_into,
        ...
        auto message = messages::GetTOFMeasurementMessage{
            .id = id, .sensor_id = gcode.sensor_id, .resend = gcode.resend};
        if (!task_registry->send(message, TICKS_TO_WAIT_ON_SEND)) {
            auto wrote_to = errors::write_into(
                tx_into, tx_limit, errors::ErrorCode::INTERNAL_QUEUE_FULL);
            get_tof_measurement_cache.remove_if_present(id);
            return std::make_pair(false, wrote_to);
        }
        return std::make_pair(true, tx_into);
    }
    ...
    requires std::forward_iterator<InputIt> &&
        std::sized_sentinel_for<InputLimit, InputIt>
    auto visit_message(const messages::GetTOFMeasurementResponse& response,
                       InputIt tx_into, InputLimit tx_limit) -> InputIt {
        auto cache_entry = get_tof_measurement_cache.remove_if_present(
            response.responding_to_id);
        return std::visit(
            [tx_into, tx_limit, response](auto cache_element) {
                using T = std::decay_t<decltype(cache_element)>;
                if constexpr (std::is_same_v<std::monostate, T>) {
                    ...
                    return cache_element.write_response_into(
                        tx_into, tx_limit, response.sensor_id, response.id,
                        response.data);
                }
            },
            cache_entry);
