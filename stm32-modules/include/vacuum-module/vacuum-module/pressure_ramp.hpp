...
public:
     ...
     * @param start_pressure The current pressure reading when starting the ramp.
     ...
     */
    auto start_ramp(double start_pressure, 
    ...
    auto update_setpoint(uint32_t current_time_ms) -> double{
        ...
        double elapsed_seconds = static_cast<double>(current_time_ms - _start_time_ms) * MS_TO_SECONDS;

        // Calculate the magnitude of pressure change requested so far
        double pressure_change_magnitude = _ramp_rate_per_second * elapsed_seconds;
        ...
        if (std::abs(ramped_setpoint - _final_target_pressure) < PRESSURE_RAMP_TOLERANCE) {
