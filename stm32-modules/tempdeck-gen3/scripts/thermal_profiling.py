...
if __name__ == '__main__':
    ...
    config = PowerConfig(power=args.power,
                         ...
                         process='temperature',
                         process_unit='C')
    tempdeck.set_peltier_power(power = config.power)
    ...
    prefix = []
    for i in range(10):
        plate, _ = tempdeck.get_temperatures()
        now = datetime.datetime.now()
        prefix.append(((now-start).total_seconds(), plate))
        time.sleep(config.sampling_time.total_seconds())
    prefix_end = datetime.datetime.now()
    data = test_utils.sample_fixed_time(config, config.duration,
                                        tempdeck.get_temperatures, done)
    try:
        tempdeck.set_peltier_power(power = 0)
    except RuntimeError:
        pass
    #port.close()
